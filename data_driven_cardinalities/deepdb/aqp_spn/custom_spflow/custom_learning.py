...
from data_driven_cardinalities.deepdb.aqp_spn.aqp_leaves import Categorical
from data_driven_cardinalities.deepdb.aqp_spn.aqp_leaves import IdentityNumericLeaf
from data_driven_cardinalities.deepdb.aqp_spn.custom_spflow.custom_rdc import getIndependentRDCGroups_py
...
def learn_mspn(
    ...
    from data_driven_cardinalities.deepdb.aqp_spn.custom_spflow.custom_structure_learning import get_next_operation, \
        learn_structure
...
def create_custom_leaf(data, ds_context, scope):
    ...
    if meta_type == MetaType.REAL:
        ...
        from data_driven_cardinalities.deepdb.aqp_spn.custom_spflow.custom_validity import is_valid_prob_sum
...
def get_split_cols_RDC_py(max_sampling_threshold_cols=10000, threshold=0.3, ohe=True, k=10, s=1 / 6,
    ...
    def split_cols_RDC_py(local_data, ds_context, scope):
        ...
        if local_data.shape[0] > max_sampling_threshold_cols:
            ...
            clusters = safeRDCclusters(domains, local_data_sample, meta_types)
            return split_data_by_clusters(local_data, clusters, scope, rows=False)
        else:
            clusters = safeRDCclusters(domains, local_data, meta_types)
            return split_data_by_clusters(local_data, clusters, scope, rows=False)

    def safeRDCclusters(domains, local_data_sample, meta_types):
        try:
            ...
            print(f"Error during RDC computation {e}")
            clusters = np.ones(local_data_sample.shape[1])
        return clusters
