...
from data_driven_cardinalities.deepdb.aqp_spn.group_by_combination import group_by_combinations
from data_driven_cardinalities.deepdb.ensemble_compilation.spn_ensemble import CombineSPN
from data_driven_cardinalities.deepdb.rspn.algorithms.ranges import NominalRange, NumericRange
from data_driven_cardinalities.deepdb.rspn.rspn import RSPN
from data_driven_cardinalities.deepdb.rspn.structure.leaves import IdentityNumericLeaf, identity_distinct_ranges, \
    categorical_distinct_ranges, \
    Categorical, identity_likelihood_range, categorical_likelihood_range
from data_driven_cardinalities.deepdb.rspn.updates.top_down_updates import cluster_center_update_dataset
...
class AQPSPN(CombineSPN, RSPN):
    ...
    def evaluate_expectation(self, expectation, standard_deviations=False, gen_code_stats=None, join_keys=None,
                             scale=1):
        """
        ...
        return self.evaluate_expectation_batch(expectation, None, None, standard_deviations=standard_deviations,
                                               gen_code_stats=gen_code_stats, join_keys=join_keys, scale=scale)

    def evaluate_indicator_expectation(self, indicator_expectation, standard_deviations=False,
                                       gen_code_stats=None, join_keys=None, scale=1):
        """
        ...
        return self.evaluate_indicator_expectation_batch(indicator_expectation, None, None,
                                                         ...
                                                         join_keys=join_keys, scale=scale)
    ...
    def evaluate_indicator_expectation_batch(self, indicator_expectation, group_bys, group_by_tuples,
                                             standard_deviations=False, gen_code_stats=None, join_keys=None, scale=1):
        """
        ...
        range_conditions = self._parse_conditions(indicator_expectation.conditions, group_by_columns=group_bys,
                                                  group_by_tuples=group_by_tuples, join_keys=join_keys, scale=scale)
    ...
    def _parse_conditions(self, conditions, group_by_columns=None, group_by_tuples=None, join_keys=None, scale=1):
        """
        ...
        for (table, condition) in conditions:
            ...
            matching_cols = []
            matching_fd_cols = []
            for op in ['<', '=', '>', ' ',  '!=', 'IS NOT NULL']:
                matching_cols += [column for column in self.column_names if column + op in table + '.' + condition]
                matching_fd_cols += [column for column in self.column_names if column + op in table + '.' + condition]
            if len(matching_cols) == 0 and len(matching_fd_cols) == 0:
                for op in ['<', '=', '>', ' ', '!=', 'IS NOT NULL']:
                    matching_cols += [column for column in self.column_names if column.lower() + op in table + '.' + condition]
                    matching_fd_cols += [column for column in self.column_names if
                                         column.lower() + op in table + '.' + condition]

            if len(matching_cols) == 2 or len(matching_fd_cols) == 2:
                # this is just a join condition
                # print(f"Skipping condition {condition} since it is a join condition")
                continue

            assert len(matching_cols) == 1 or len(
                matching_fd_cols) == 1, f"Found multiple or no matching columns for {table + '.' + condition}"
            ...
            attribute_index = self.column_names.index(matching_column)
            join_key = False
            if scale > 1 and table in join_keys and matching_column.split('.')[1] in join_keys[table]:
                join_key = True
            ...
            if self.meta_types[attribute_index] == MetaType.DISCRETE:
                ...
                if '!=' in condition:
                    ...
                    if join_key:
                        ...
                        prob_offset = 1 - 1 / scale

                    possible_values = [val_dict[literal] for literal in val_dict.keys() if
                                       not literal in literal_list and self.null_values[attribute_index] != val_dict[
                                           literal]]

                    single_range = NominalRange(possible_values, prob_offset=prob_offset, scale_factor=scale_factor)
                    if all([single_range is None for single_range in ranges[:, attribute_index]]):
                        ...
                        for i, nominal_range in enumerate(ranges[:, attribute_index]):
                            ranges[i, attribute_index] = NominalRange(
                                ...
                                prob_offset=prob_offset, scale_factor=scale_factor)
                    ...
                    scale_factor = 1
                    if join_key:
                        # equality condition is less likely for larger domain
                        scale_factor = 1 / scale

                    if literal in val_dict:
                        if group_by_columns_merged is None or matching_column not in group_by_columns_merged:
                            ranges[:, attribute_index] = NominalRange([val_dict[literal]], scale_factor=scale_factor)
                        else:
                            ...
                            for idx in range(len(ranges)):
                                ...
                                ranges[idx, attribute_index] = NominalRange([literal], scale_factor=scale_factor)
                ...
                if '!=' in condition:
                    ...
                    if group_by_columns_merged is None or matching_column not in group_by_columns_merged:
                        ranges[:, attribute_index] = NumericRange([[literal, literal]], negated=True,
                                                                  prob_offset=prob_offset, scale_factor=scale_factor)
                    else:
                        matching_group_by_idx = group_by_columns_merged.index(matching_column)
                        ...
                        for idx in range(len(ranges)):
                            literal = group_by_tuples[idx][matching_group_by_idx]
                            ranges[idx, attribute_index] = NumericRange([[literal, literal]], negated=True,
                    ...
                    try:
                        ...
                        map(non_conflicting, ranges)
                        if group_by_columns_merged is None or matching_column not in group_by_columns_merged:
                            ranges[:, attribute_index] = NumericRange([[literal, literal]], scale_factor=scale_factor)
                        else:
                            matching_group_by_idx = group_by_columns_merged.index(matching_column)
                            assert literal == group_by_tuples[0][matching_group_by_idx]
                            for idx in range(len(ranges)):
...
def _adapt_ranges(attribute_index, literal, ranges, inclusive=True, lower_than=True, scale_factor=1, prob_offset=0):
