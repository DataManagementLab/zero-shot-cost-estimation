...
def identity_likelihood_range(node, data, dtype=np.float64, **kwargs):
    ...
    sf = ranges[0].scale_factor
    assert all([rang.scale_factor == sf for rang in ranges])
    if sf > 1:
        probs[i] *= sf
    elif sf < 1:
        probs[i] *= sf

    offset = ranges[0].prob_offset
    assert all([rang.prob_offset == offset for rang in ranges])
    probs[i] += offset
