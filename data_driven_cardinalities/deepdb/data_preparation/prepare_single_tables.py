...
def read_table_csv(table_obj, **read_kwargs):
    """
    Reads csv from path, renames columns and drops unnecessary columns
    """
    df_rows = pd.read_csv(table_obj.csv_file_location, **read_kwargs)

    df_rows.columns = [table_obj.table_name + '.' + attr for attr in table_obj.attributes
                       if attr not in table_obj.compound_keys]

    for attribute in table_obj.irrelevant_attributes:
        df_rows = df_rows.drop(table_obj.table_name + '.' + attribute, axis=1)

    for comp_key in table_obj.attributes:
        if comp_key in table_obj.compound_keys:
            key_parts = table_obj.compound_keys[comp_key]
            comp_keys = None
            for c in key_parts:
                if comp_keys is None:
                    comp_keys = df_rows[f'{table_obj.table_name}.{c}'].astype(str)
                else:
                    comp_keys += '_' + df_rows[f'{table_obj.table_name}.{c}'].astype(str)
            df_rows[table_obj.table_name + '.' + comp_key] = comp_keys
...
def prepare_single_table(schema_graph, table, path, read_kwargs, max_distinct_vals=10000, max_table_data=20000000):
    """
    Reads table csv. Adds multiplier fields, missing value imputation, dict for categorical data. Adds null tuple
    tables.
    ...
    table_data = read_table_csv(table_obj, **read_kwargs)
    ...
    for relationship_obj in incoming_relationships:
        ...
        neighbor_table_data = read_table_csv(neighbor_table_obj, **read_kwargs).set_index(right_attribute, drop=False)
        ...
        muls = table_data.join(neighbor_table_data, how='left')[[left_attribute, right_attribute]] \
            .groupby([left_attribute]).count()
        ...
        print(f"Median value {mu_col_name}: {np.median(muls[mu_col_name])}")
        print(f"Mean value {mu_col_name}: {np.mean(muls[mu_col_name])}")
    ...
    for rel_attribute in relevant_attributes:
        ...
        if table_data.dtypes[attribute] == object:
            ...
            if len(distinct_vals) > max_distinct_vals and rel_attribute not in schema_graph.join_keys[table]:
    ...
    for relationship_obj in incoming_relationships:
        ...
        table_data = table_data.set_index(left_attribute, drop=False)
        neighbor_table_data = read_table_csv(neighbor_table_obj, **read_kwargs)

        # in case join column is a categorical column similarly transform here
        if left_attribute in table_meta_data['categorical_columns_dict']:
            val_dict = table_meta_data['categorical_columns_dict'][left_attribute]
            neighbor_table_data[right_attribute] = neighbor_table_data[right_attribute].map(val_dict.get)

        neighbor_table_data = neighbor_table_data.set_index(right_attribute, drop=False)

        null_tuples = table_data.join(neighbor_table_data, how='left')
        null_tuples = null_tuples.loc[null_tuples[right_attribute].isna(),
...
def prepare_all_tables(schema_graph, path, max_table_data=20000000):
    ...
    for table_obj in schema_graph.tables:
        ...
        meta_data[table] = prepare_single_table(schema_graph, table, path + '/' + table + '.hdf',
                                                schema_graph.read_kwargs, max_table_data=max_table_data)
